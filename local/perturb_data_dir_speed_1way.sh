...
if $always_include_prefix; then
  ...
  utils/data/combine_data.sh $destdir ${destdir}_speed1.0 ${destdir}_speed1.1 || exit 1

  rm -r ${destdir}_speed1.1 ${destdir}_speed1.0
else
  utils/data/combine_data.sh $destdir ${srcdir} ${destdir}_speed1.1 || exit 1
  rm -r ${destdir}_speed1.1
fi
